...
class TechnoVoiceNode(Node):
    def __init__(self) -> None:
        ...
        # --- NEW VARIABLES FOR WEB TOGGLE ---
        self._is_awake = False
        self._is_listening_enabled = False # Starts disabled until web button is clicked
        self._toggle_srv = self.create_service(SetBool, 'toggle_techno_voice', self._toggle_voice_cb)
        ...
        self.get_logger().info('🤖 AI Techno voice ready! (Currently disabled. Enable via Web GUI)')

    def _toggle_voice_cb(self, request, response):
        with self._state_lock:
            self._is_listening_enabled = request.data
            if not self._is_listening_enabled:
                self._is_awake = False
                self._command_buffer = ""
        
        state_str = "ENABLED" if self._is_listening_enabled else "DISABLED"
        self.get_logger().info(f'🎤 Voice System {state_str}')
        response.success = True
        response.message = f"Voice system is now {state_str}"
        return response
    ...
    def _audio_loop(self) -> None:
        ...
        self._silence_send_timeout_sec = 10.0 
        ...
        while not self._shutdown.is_set():
            ...
            # Check if web GUI disabled listening
            ...
            if self._is_awake and not self._command_buffer and (time.time() - self._last_voice_time > self._idle_timeout_sec):
                self._is_awake = False
                self._command_buffer = ""
                self.get_logger().info("💤 Silence timeout. Going back to sleep.")
                continue
            ...
            if self._command_buffer.strip() and (time.time() - self._last_voice_time > self._silence_send_timeout_sec):
                ...
                self._command_buffer = ""
                self.get_logger().info(f'🚀 Sending Full Sentence to AI: "{final_command}"')
            ...
            if rec.AcceptWaveform(data):
                ...
                if not self._is_awake:
                    ...
                    if any(w in words for w in wake_words):
                        ...
                        self.get_logger().info('🟢 Woke up! Listening...')
                        
                else:
                    if any(w in norm for w in sleep_words):
                        self._is_awake = False
                        self.get_logger().info('🔴 Sleep command heard.')
